...
- **2026-07-06 (Multimedia en Talleres). DEPLOY COMPLETADO:** Se dotó al catálogo de talleres (`Workshops.jsx`) de capacidades multimedia heredadas del módulo de Avisos. Se actualizaron las reglas de `firebase storage` para admitir subidas en el bucket `/workshops/`. Ahora se pueden adjuntar imágenes de portada que se despliegan con el formato cinemático `aviso-cover` (fondo 16:9 desenfocado) en la tarjeta del catálogo. Además, se permite adjuntar múltiples archivos (PDFs, Excel, Word, imágenes) como temarios o listas de materiales que se muestran con su icono correspondiente y botón de descarga directa. Se implementaron los scripts de subida en `utils/workshops.js`.
